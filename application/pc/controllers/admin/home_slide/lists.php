...
class Lists extends CI_Controller {
	...
	//	echo $setData['page'];
		...
		if($search){
			$setData['session']['where']['title_vn'] =  $this->input->post('fTitle');
			...
			if($setData['session'] == FALSE):
				$setData['session']['where']['title_vn']  = '';
		...
		if( !empty($setData['session']['where']['id']) || 
			!empty($setData['session']['where']['title_vn'])  ||
			...
			$whereLike['title_vn']        =  $setData['session']['where']['title_vn'];
			$this->session->set_userdata($setData['page'], $setData['session']);
	...
	function showTable($value){
		...
		// SET VALUE PAGE
		$data["page_title"]       = $value["lang"]=="en" ? "Slideshow" : "Hình nh";
		$data["page_table_sql"]   = $value["page"];
		

		$data['template'] 		  = "admin/home_slide/".$value['action'].".php";		
