...
class Banner extends CI_Controller {
	...
	public function SavePhoto($setData){
		...
		for($i=0; $i<count($id);$i++):
			...
			if($delete==1):
				$this->admindino->DeleteItem($setData['TABLESQL'].'_banner',$id[$i]);
				...
				$this->admindino->UpdateTable($setData['TABLESQL'].'_banner',$array,$id[$i]);
			endif;
		endfor;
	...
	public function SaveItemPhoto($setData){
		...
		$this->admindino->UpdateTable($setData['TABLESQL'].'_banner',$array,$id);
	...
	public function EditPhoto($data){
		$id = $data['id'];
		$data['result'] 	  = $this->admindino->GetValueFrom($data['TABLESQL'].'_banner','id',$id,'*');	
		///XML DATA
	...
	public function loadPhoto($setData){
		...
		'album_id'   => $id,
		'database'   => $setData['TABLESQL'].'_banner',
