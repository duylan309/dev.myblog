...
class Lists extends CI_Controller {
	...
	public function searchTable($setData){
		...
		if( !empty($setData['session']['where']['id']) || !empty($setData['session']['where']['title_'.$setData['lang']])  || $setData['session']['status']>=0 ||!empty($setData['session']['sortTable']) ):
			...
			$setData['coutAll']  = $this->admindino->countSearch($setData["TABLESQL"],$whereLike,$whereValue);
		else:
	...
	function showTable($value){
		...
		$data["results"]           = $this->admindino->ListItems($value["TABLESQL"],$config["per_page"], $page ,$data['session']['where'],$whereValue,$sort,$sortValue);//Get result from search
		...
		$data["page_table_sql"]   = $value["TABLESQL"];
		
		$data['template'] 		  = "admin/".$value["FOLDERCONTROL"]."/".$value['action'].".php";		
