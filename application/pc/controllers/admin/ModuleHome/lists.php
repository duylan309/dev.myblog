...
class Lists extends CI_Controller {
	...
	//	echo $setData['page'];
		...
		if( !empty($setData['session']['where']['id']) || !empty($setData['session']['where']['title_'.$setData['lang']])  || $setData['session']['status']>=0  || $setData['session']['sort']>=0):
			...
			$this->session->set_userdata($setData['page'], $setData['session']);
			$setData['coutAll']  = $this->admindino->countSearch(strtolower($setData['page']),$whereLike,$whereValue);
		else:
	...
	function showTable($value){
		...
		$data['template'] 		  = "admin/ModuleHome/".$value['action'].".php";		
