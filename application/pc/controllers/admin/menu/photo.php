...
class Photo extends CI_Controller {
	...
	public function SaveItemPhoto($setData){
		...
		redirect(base_url().ADMINBASE.'?page='.$setData['page'].'&action=gallery&id='.getParam($this,'parent_id').'&function=photo');
	}
	...
	public function loadPhoto($setData){
		...
		//'user_dirs'=> true,
		'delete_type' => 'DELETE',
		'script_url' => './upload/storage/'.$setData['page'].'/'.$id.'/',
		'album_id'   => $id,
		'database'   => $setData['page'].'_photo',
		'model_name' => 'admindino',
		'model_url'  => 'admin/admindino',
		'savetodatabase' =>1,
		'upload_dir' => './upload/storage/'.$setData['page'].'/'.$id.'/',
		'upload_url' => './upload/storage/'.$setData['page'].'/'.$id.'/',
		);
