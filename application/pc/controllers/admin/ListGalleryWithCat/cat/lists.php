...
class Lists extends CI_Controller {
	...
	//	echo $setData['page'];
		...
		if( !empty($setData['session']['where']['id']) || !empty($setData['session']['where']['title_'.$setData['lang']])  || $setData['session']['status']>=0 || $setData['session']['cat']>=0):
	...
	function showTable($value){
		...
		$data['result_cat']        = $this->admindino->loadTable($value['page'],NULL,NULL);// Get resutl 		
		...
		$data['template'] 		  = "admin/ListGalleryWithCat/cat/".$value['action'].".php";		
