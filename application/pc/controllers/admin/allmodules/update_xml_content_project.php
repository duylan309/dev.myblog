...
$file_xml_vn = $setData['define_folder']['xml'].$setData['define_folder']['vietnam'].'/'.$setData['page'].'/'.$id.".xml";
			...
			$information_en = $this->dinosaur_lib->xml_post_project(	stripcslashes($data['title_en']),
																stripcslashes($_REQUEST['hotline']),
																!empty($_REQUEST["download"])? stripcslashes($_REQUEST["download"]) : '&nbsp;',
			...
			$information_vn = $this->dinosaur_lib->xml_post_project(	stripcslashes($data['title_vn']),
																stripcslashes($_REQUEST['hotline']),
																!empty($_REQUEST["download"])? stripcslashes($_REQUEST["download"]) : '&nbsp;',
