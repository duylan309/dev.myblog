...
class Action extends CI_Controller {
	...
	{
		$data['FOLDERXML']     = XMLCARSTYLE;
		$data['FOLDERIMAGE']   = IMAGECARSTYLE;
		$data['TABLESQL']      = TABLECARSTYLE;
		$data['FOLDERCONTROL'] = FOLDERCONTROLCARSTYLE;
		
		$data["POSITIONIMG"]   = isset($_GET['position']) && count($_GET['position']) ? $_GET['position'] : '';
		if($data['POSITIONIMG']=="gallery"):
			$data['TABLEGALLERY'] = TABLECARSTYLE.'_photo';
		elseif($data['POSITIONIMG'] == "banner"):
			$data['TABLEGALLERY'] = TABLECARSTYLE.'_banner';
		endif;	
