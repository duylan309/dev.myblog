...
{   
	...
	public function UpdateTableWhere($database,$data,$where){
	...
	public function DeleteTableWhere($database,$where){
		if($where != null){
			foreach ($where as $gWhere => $val){
				if($val >=0)
				$this->db->where($gWhere,$val);	
	...
	public function countSearchNotIn($database,$wherelike, $wherevalue,$array=array()){
	...
	public function ListItemsNoIn ($database,$limit, $start, $whereLike = NULL,$whereValue = NULL, $sort, $valueSort,$array=array()){
		$this->db->select('*');
		if($whereLike!=NULL){  
			$i=0; 
			foreach($whereLike as $getWhere => $value){
	...
	public function ListItemsJoinNoLimit($database,$database_cat,$select,$where_join, $whereLike = NULL,$whereValue = NULL, $sort, $valueSort){
		$this->db->select($select);
		...
		$this->db->join($database_cat, $where_join, 'inner');
			...
			$this->db->order_by($database.'.'.'date', 'DESC'); 
