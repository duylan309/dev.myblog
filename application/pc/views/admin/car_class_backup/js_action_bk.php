<script type="text/javascript">
 ...
 // ADD BUTTON
    ...
    $('#addTabButton').on('click',function() {
      ...
      var tab_title_en = $('[data-tab-title-en]').val();

      if(tab_title_en == "" || tab_title_vn == ""){
        $('[data-tab-title-vn]').addClass('require');
        $('[data-tab-title-en]').addClass('require');
        return false;
      }

      var tab_url      = urlFriendly(tab_title_en);
      ...
      $('#'+tab_url+' #en_new_tab_added input').attr('value',tab_title_en);
      $('#'+tab_url+' [data-url]').attr('value',tab_url);

      // SET TAB SELECTION
      $('#'+tab_url+' [data-title] li:nth-child(1) a').attr('href','#vn_title_'+tab_url);
      $('#'+tab_url+' [data-title] li:nth-child(2) a').attr('href','#en_title_'+tab_url);
      $('#'+tab_url+' [data-title] .tab-pane:nth-child(1)').attr('id','vn_title_'+tab_url);
      $('#'+tab_url+' [data-title] .tab-pane:nth-child(2)').attr('id','en_title_'+tab_url);

      $('#'+tab_url+' [data-content] li:nth-child(1) a').attr('href','#vn_content_'+tab_url);
      $('#'+tab_url+' [data-content] li:nth-child(2) a').attr('href','#en_content_'+tab_url);
      $('#'+tab_url+' [data-content] .tab-pane:nth-child(1)').attr('id','vn_content_'+tab_url);
      $('#'+tab_url+' [data-content] .tab-pane:nth-child(2)').attr('id','en_content_'+tab_url);
      ...
      $('[data-tab-title-en]').removeClass('require').val('');
