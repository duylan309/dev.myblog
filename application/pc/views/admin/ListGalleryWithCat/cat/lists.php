  ...
  <div class="subhead">
    <ul>
      <li style="float:left"> <a href="#" class="btn clearmenu">
        <h2 style="color:#fff;font-size:10pt;margin-left:20px">
        <?=$lang=="en" ? $getMenuAdmin->title_en:$getMenuAdmin->title_vn?>
          &nbsp;&nbsp;&nbsp; -&nbsp;&nbsp;&nbsp;</h2>
        </a> </li>
      ...
      <li> <a href="<?=base_url().ADMINBASE?>?page=<?=$page?>&action=update&id=0&function=Save">
        <label id="add_new" class="btn">
  ...
  <div class="listContent">
	...
	?>
          <tr id="<?=$row->id?>">
           ...
           <!-- <td><select style="width:160px" onchange="changeCat(<?=$row->id?>,'<?=$page?>','<?=base_url()?>')" class="changcat" name="cat" id="cat">
                <option value="0" <?=$row->cat==0 ? "selected" : ""?>>Parent</option>
                <?php foreach($arrCat as $key ):?>
                <?php if($key['cat'] != $row->id && $key['source'] != $row->id):?>
                <option <?=$key['id'] == $row->id ?'style="background:#333;color:#fff"' :'' ?> value="<?=$key['id']?>" <?=$key['id'] == $row->cat ? "selected" : ""?>> <?=$lang=='en' ? $key['text_en']: $key['text_vn']?></option>
                <?php endif;?>
                <?php endforeach;?>
              </select></td>-->
            <td><?=$row->date?></td>
            <td><img class="thumbnail" src="<?=base_url().'upload/'.$page.'/'.$row->image?>"  /></td>
            <td class="col6"><span>
              <input class="changesort" value="<?=$row->sort?>" page="<?=$page?>" onchange="changeSort(<?=$row->id?>,'<?=$page?>','<?=base_url()?>',0,0)" />
              </span>
              </label></td>
            <td class="col7"><label onclick="changePublish(<?=$row->id?>,'<?=$page?>','<?=base_url()?>')" class="iconSetup">
