...
class Uploadhandler_lib {
	...
	protected function saveDatabase($model,$filename){
		$CI =& get_instance();
   	    $CI->load->model($this->options['model_url']);  //<-------Load the Model first
		...
		$new['newid']       = $CI->$model->AddPhoto($this->options['database'],$array);
		$new['newposition'] = $CI->$model->CountPhoto($this->options['database'],'album_id',$this->options['album_id']);
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
		...
		$file->alt_image = 'alt_image';
		$file->content   = 'add your content';
		$file->linkphoto = '#';
		$file->sort      = '0';
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                    ...
                    if(move_uploaded_file($uploaded_file, $file_path)):
						if($this->options['savetodatabase'] == 1):
						$new = $this->saveDatabase($this->options['model_name'],$file->name);
						$file->newid = $new['newid'];
						$file->newposition = $new['newposition']-1;
						endif;
     				endif;
	            }
            } else {
    ...
    public function _delete($print_response = true) {
        $CI =& get_instance();
   	    $CI->load->model($this->options['model_url']); 
	    $model = $this->options['model_name'];
		
		$file_name = $this->options['delete_photo_id'];
        
		$imagename = $CI->$model->getNameImage(strtolower($this->options['database']),$file_name,$this->options['album_id']);
		$file_path = $this->options['upload_dir'].$imagename->name;				
		$success = is_file($file_path) && $imagename->name !== '.' && unlink($file_path);
			if ($success) {
				$file =  $this->options['upload_dir'].'thumbnail/'.$imagename->name;
				if (is_file($file)) {
					if(unlink($file)){
						
						$CI->$model->deleteImage(strtolower($this->options['database']),$imagename->name,$this->options['album_id']);
					}
				}
			}
