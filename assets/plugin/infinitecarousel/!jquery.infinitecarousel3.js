		...
		infiniteCarousel: function(options)
		{
			...
			var options = $.extend(defaults, options);
				...
				$('li',obj).each(function(index){ // populate arrays
					captions.push( $('div',this).html() );
					links.push( $('a',this).attr('href') );
					vidThumb.push( $(this).find('iframe').attr('longdesc') );
					itemSources.push( $(this).find('img.bgimage, iframe').attr('src') ); // finds images and youtube and vimeo iframe sources
					itemWidthPush.push( $(this).find('img.bgimage, iframe').attr('width') );
					itemHeightPush.push( $(this).find('img.bgimage, iframe').attr('height') );
					itemId.push( $(this).find('img.bgimage, iframe').attr('data_id'));
					altSources.push( $(this).find('img.bgimage, iframe').attr('alt') );
				});
			...
			//	heightSource = $(this).find('img, iframe').height();
				...
				widthSource = $(window).width();
				heightSource = $(window).height();
		
			/*	$(window).resize(function() {
					widthSource = $(window).width();
					heightSource = $(window).height();
					$('.ic_wrapper,.ic_peek_padding,')
				});	*/
				...
				function setAllSize(widthx,heightx){
					$(obj).append('<div class="ic_tray" style="position:relative;width:'+(numItems*2)*widthx+'px;left:-'+widthx+'px">');
					for(var i=0;i<numItems;i++) $('div.ic_tray',obj).append('<div style="overflow:hidden;background:url('+o.imagePath+'wait.gif) no-repeat scroll 50% 50% #ddd;float:left;position:relative;width:'+widthx+'px;height:'+heightx+'px;" class="infiniteCarousel_item">');
					
				}
				
				function resizeImg(valWidth,valHeight) {
					 ...
					 // alert(imgwidth);
					  ...
					  if(heightdiff>winheight) {
						  valueImg.push(winwidth);
						  valueImg.push(heightdiff);
					//	});
					  } else {
						  valueImg.push(widthdiff);
						  valueImg.push(winheight);
					  }
					  return valueImg;	
					}
				
				setAllSize(widthSource,heightSource);
				...
				$('.infiniteCarousel_item',obj).each(function(index){
					...
					{	
					  	var SizeSet = resizeImg(itemWidthPush[index],itemHeightPush[index]);
					  
						$(this).append('<img width="'+SizeSet[0]+'" height="'+SizeSet[1]+'" class="countImg itemImg_'+index+'" src="'+itemSources[index]+'" />');
					...
					if(captions[index]!=undefined) $(this).append('<div class="ic_caption control_'+itemId[index]+'" style="position:fixed;left:0;display:none">'+captions[index]+'</div>');
				});
				...
				//alert(index)
				$(obj).append('<div class="ic_left_nav" style="z-index:10;position:fixed;right:0;width:40px;top:'+(heightSource/2-0)+'px;">').append('<div class="ic_right_nav" style="z-index:10;position:fixed;width:40px;right:0;top:'+(heightSource/2-50)+'px;">');
				...
				$('.ic_left_nav',obj).on('click', function(event){
					if( !$('.ic_tray',obj).is(':animated') )
					{
					 ...
					 $('.article_img').animate({opacity:0},500,function(){$('.description,.imgArticle').html('')});
					var getId = $('.ic_left_nav').attr('next_id').split('/');
					$('.ic_caption,.col2').css('display','none');
					$('.control_'+getId[0]).eq(1).css('display','block').find($('.scroll-pane')).jScrollPane();
					 window.location.href =  '#/'+altSources[parseInt(getId[1])];
					if(parseInt(getId[1]) == itemId.length-1){
								$('.ic_left_nav').attr('next_id',itemId[parseInt(getId[1])-1]+'/'+(parseInt(getId[1])-1));
					    		$('.ic_right_nav').attr('next_id',itemId[0]+'/0');
							}else if(getId[1] == 0){
								$('.ic_left_nav').attr('next_id',itemId[itemId.length-1]+'/'+(itemId.length-1));
					    		$('.ic_right_nav').attr('next_id',itemId[parseInt(getId[1])+1]+'/'+(parseInt(getId[1])+1));
							}else{
				...
				$('.ic_right_nav img',obj).on('click', function(event){
					if( !$('.ic_tray',obj).is(':animated') )
					{
					 ...
					 $('.article_img').animate({opacity:0},500,function(){$('.description,.imgArticle').html('')});
					var getId = $('.ic_right_nav').attr('next_id').split('/');
					$('.ic_caption,.col2').css('display','none');
					$('.control_'+getId[0]).eq(1).css('display','block').find($('.scroll-pane')).jScrollPane();
					 window.location.href =  '#/'+altSources[parseInt(getId[1])];
					if(parseInt(getId[1]) == itemId.length-1){
				...
				else
				{
					...
					$(obj).hover(function(){ 
						...
						$('.ic_left_nav',obj).stop().animate({opacity:0,left:'-40px'},500);
					$('.ic_right_nav',obj).stop().animate({opacity:0,right:'-340px'},500);
					});
				}
				...
				//$(obj).width() + parseInt($('#ic_'+randID).css('padding-left')) + parseInt($('#ic_'+randID).css('padding-right'))

				// Thumbnails
				if(o.thumbnailType != 'none')
				{
					// Add a tray div for the thumbnails to exist in. If using external thumbnails, move div in DOM.
					$(obj).append('<div id="ic_'+randID+'_thumbnail_tray" class="ic_thumbnail_tray" style="position:absolute;bottom:0;width:100%;text-align:center;">');
					...
					$('.ic_button').hover(function(){ if( !$(this).hasClass('ic_active') ) $(this).css('background','none') },function(){$(this).css('background','')});
					...
					if(o.thumbnailType == 'menu')
					{
						// insert images in the ic_button divs with width/height set to 100%. need to set width and height on .ic_button in css
						$('.ic_button',obj).each(function(index){
							if( altSources[index]!=undefined ) thumbImage = altSources[index];
							if(index == itemSources.length-1){
								$(this).append('<a data_id="'+itemId[index]+'" data_position="'+index+'" class="button" id="'+thumbImage+'" href="#/'+thumbImage+'">'+thumbImage+'</a> ');
							}else{					
							    $(this).append('<a data_id="'+itemId[index]+'" data_position="'+index+'" class="button" id="'+thumbImage+'" href="#/'+thumbImage+'">'+thumbImage+' /</a> ');
							}
						});
					...
					{
						$('.ic_button',obj).each(function(index){
							$(this).html('<a href="#'+menuArray[index+1]+'">'+menuArray[index+1]+'</a>');
						});
					}
					...
					var getValueUrl    = window.location.href.split('/');
				
					if(getValueUrl[5]){
						    var indexValue =  $('.ic_button a').index($('#'+getValueUrl[5]));
							
							var getId = $('#'+getValueUrl[5]).attr('data_id');
							$('.control_'+getId).eq(1).css('display','block').find($('.scroll-pane')).jScrollPane();
							$('.control_'+getId).eq(1).find($('.scroll-pane-2')).jScrollPane();
							if(indexValue == itemId.length-1){
								$('.ic_left_nav').attr('next_id',itemId[indexValue-1]+'/'+(indexValue-1));
					    		$('.ic_right_nav').attr('next_id',itemId[0]+'/0');
							}else if(indexValue == 0){
								$('.ic_left_nav').attr('next_id',itemId[itemId.length-1]+'/'+(itemId.length-1));
					    		$('.ic_right_nav').attr('next_id',itemId[indexValue+1]+'/'+(indexValue+1));
							}else{
								$('.ic_left_nav').attr('next_id',itemId[indexValue-1]+'/'+(indexValue-1));
					    		$('.ic_right_nav').attr('next_id',itemId[indexValue+1]+'/'+(indexValue+1));
							}
					...
					$('.ic_button',thumbTopParent).on('click', function(){
						if( !$('.ic_tray',obj).is(':animated') )
						{
							...
							if(jumpFinish == itemId.length-1){
								$('.ic_left_nav').attr('next_id',itemId[jumpFinish-1]+'/'+(jumpFinish-1));
					    		$('.ic_right_nav').attr('next_id',itemId[0]+'/0');
							}else if(jumpFinish == 0){
								$('.ic_left_nav').attr('next_id',itemId[itemId.length-1]+'/'+(itemId.length-1));
					    		$('.ic_right_nav').attr('next_id',itemId[jumpFinish+1]+'/'+(jumpFinish+1));
							}else{
								$('.ic_left_nav').attr('next_id',itemId[jumpFinish-1]+'/'+(jumpFinish-1));
					    		$('.ic_right_nav').attr('next_id',itemId[jumpFinish+1]+'/'+(jumpFinish+1));
							}
				...
				if(canvasSupported)
				{
					$(obj).append('<canvas id="ic_canvas_'+randID+'" width="30" height="30" style="position:absolute;top:0;right:0;display:none;"></canvas>');
